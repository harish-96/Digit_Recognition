...
class NN_hwr(object):
    ...
    def train_batch(self, batch, learning_rate):
        ...
        :param ndarray batch: An array of training examples with each being a tuple containing the input data and its label.
        :param fload learning_rate: The learning rate which determines the step size in gradient descent
...
if __name__ == '__main__':
    ...
    X_train_bin = 0.5 * (1 + np.sign(X_train - 0.5))
    X_test_bin = 0.5 * (1 + np.sign(X_test - 0.5))
    display_data(X_train_bin[:10], 2, 5)

    nn = NN_hwr([len(X_train[0]), 15, 10])
    nn.train_nn(X_train_bin, y_train, 10, 20, 0.06)

    accuracy = 0
    for i in range(len(X_test[:100])):
        out = nn.forward_prop(X_test_bin[i])[0][-1]
